...
1. Read mailbox profile context first when available so the request is grounded in the signed-in identity, locale, and default assumptions for the calendar.
2. Resolve calendar scope before reasoning. If multiple calendars are available, identify the intended one explicitly and prefer the default personal calendar unless the user names a shared, delegated, or team calendar.
3. Read the relevant calendar state so the request is grounded in actual events, attendee responses, calendars, and time windows.
4. Normalize relative time language into explicit dates, times, and timezone-aware ranges before reasoning about availability.
5. Start from Outlook's scheduling surfaces, not generic calendar abstractions. Account for attendee response state, organizer vs attendee role, shared-calendar visibility, work hours, work location, and room/resource context when available.
6. Surface Outlook-specific status semantics before proposing changes. Call out whether a block is `Tentative`, `Free`, `Out of Office`, `Working Elsewhere`, or only visible as shared-calendar busy time.
7. When the request depends on workday norms, prefer Outlook-native cues such as work hours, location plans, and partial free/busy visibility over generic assumptions about "focus time".
8. When notes, Teams links, rooms, or missing details matter, inspect the event payload before proposing a change and say when the source data appears partial.
9. When the request is ambiguous, summarize the best candidate slots or the exact event diff before drafting any write.
10. Treat missing title, attendees, location, Teams link, or timezone as confirmation points rather than assumptions.
11. Only create, update, move, or cancel events when the user has clearly asked for that action or confirmed the exact event details.

## Outlook Product Differences

- Outlook users often reason from free/busy and shared-calendar views, not from fully open peer calendars. When visibility is partial, present the answer as a free/busy recommendation rather than pretending to know the meeting titles.
- Outlook can expose multiple personal, delegated, and shared calendars in one mailbox surface. Calendar choice is part of the task, not a detail to gloss over.
...
- If multiple calendars or similarly named events are in play, identify the intended one explicitly before editing and do not silently assume the right target.
